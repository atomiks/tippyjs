...
export default function computeArrowTransform(popper, arrow, arrowTransform) {
  ...
  const computedTransform = arrowTransform.replace(
    re.translate,
    `translate${ transformAxisBasedOnPlacement(matches.translate.axis, isVertical) }(${
      transformNumbersBasedOnPlacement('translate', matches.translate.numbers, isVertical, isReverse)
    })`
  )
  .replace(
    re.scale,
    `scale${ transformAxisBasedOnPlacement(matches.scale.axis, isVertical) }(${
