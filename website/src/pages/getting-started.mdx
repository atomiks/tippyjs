...
development mode, you'll get helpful warning messages. When deploying for
production, your minifier should remove the development warnings. If you're
getting an error message about `process` inside the browser,
...
The `tippy.js` package is generally used via Node using a module bundler, or in
...
- `dist/tippy.js` = Core JS only (default for ESM/CJS versions)
- `dist/tippy.css` = Core CSS only

If you have CSP enabled, you should use `dist/tippy.js` and link the stylesheet
`dist/tippy.css` separately instead of relying on injection.
