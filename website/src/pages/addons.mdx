...
Note that the only prop `createSingleton` accepts at this time is `delay`. It
can take the same values as `tippy()`'s `delay` prop.
...
<Demo>
  <TippySingleton delay={[200, 400]}>
    <Tippy content="Hello!" updateDuration={400} animateFill={false}>
      <Button>Text</Button>
    </Tippy>
    <Tippy
      content="Hello, there!"
