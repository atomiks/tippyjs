...
moving the cursor between each item in the group, the tooltips will instant
show.
...
situations - it depends on the UI.
...
tippy('#unintrusive-example', {
  boundary: 'window',
  // Prevents the tooltip from showing outside the scrolling area
  appendTo: 'parent',
  popperOptions: {
    modifiers: {
      flip: {
...
<Scroller type="Scroll flipping" flipScroll />
