...
tippy(targets, {
  // default
  aria: {
    // aria-describedby, `null` when interactive: true
    content: 'describedby',
    // aria-expanded only when interactive: true
  ...
  // hoverable if it's not actually interactive, so the content will still be
  // announced
  aria: {
    expanded: false
  }
