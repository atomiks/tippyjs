...
These defaults can be overridden if necessary. Specifying `null` for either
property means you want Tippy to figure out the configuration automatically
...
tippy(targets, {
  ...
  // hoverable if it's not actually interactive
  aria: {
    expanded: false
  }
});
