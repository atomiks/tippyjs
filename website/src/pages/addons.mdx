...
> **Important note:** enabling transitions may cause overflow issues if the
> tippy content is near the right/bottom edge of a boundary (e.g. window) and
> the tippy content changes from small to large. This is because of the
> transition causing the tippy to expand the boundary, making Popper.js think it
> will fit.
...
If the first solution is not suitable (i.e. you prefer the `window` boundary),
you can change your DOM context to prevent the boundary from expanding by
placing the tippy within a positioned container with `overflow-x: hidden`.
