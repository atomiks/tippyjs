...
Tippy elements use a simple CSS transition by default. The main `index.css` file
includes two animations: `fade` and `shift-away`.

### Usage

Pass the animation name (string) as the `animation` option:
...
If the default packaged animations don't suit your use case, you can create your
own.
...
There's an option named `inertia` that adds an elastic inertial effect to the
...
You can customize this option in your CSS:
...
> ⚠️ **Highly experimental code.** There is no simple wrapper API available for
> this as I've only recently started working on it.
...
We're going to need the help of an external library that allows
[FLIP](https://medium.com/outsystems-experts/flip-your-60-fps-animations-flip-em-good-372281598865)
animations, which is a highly performant technique to make layout transitions
happen at 60 FPS.
