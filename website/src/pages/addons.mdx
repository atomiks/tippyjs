...
should act as a delegate, and a `target` option representing a CSS selector that
...
<script src="https://unpkg.com/tippy.js@5/iife/tippy-addons.min.js"></script>
<script>
...
If you want to clean up all the child tippy instances that were created, pass
`true` as an argument to `destroy()`.
...
<script>
  const tippyInstances = tippy('button')
  const singleton = tippy.createSingleton(tippyInstances, { delay: 1000 })
</script>
```

Note that the only option `createSingleton` accepts at this time is `delay`. It
can take the same values as `tippy()`'s `delay` option.

#### Smooth transitions

Utilize the `updateDuration` option, which is the transition duration between
position updates of the tippy element:

```js
const tippyInstances = tippy('button', { updateDuration: 500 })
```
