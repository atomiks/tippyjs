...
> **Note**: If `interactive: true`, then by default the tippy is appended to the
> reference element's parentNode instead.
...
of a selector in the form `.tippy-tooltip.x-theme`. Let's demonstrate
...
To color an SVG arrow, it's as simple as specifying the `fill` and targeting
`.tippy-svg-arrow`:
