...
If you would like your dropdown to be triggered by a hover instead of a click,
still include click as part of the trigger for keyboard users:
`mouseenter click`. The `focus` event is not suitable for buttons.
...
const instance = tippy(someButton)
instance.popper.setAttribute('role', 'menu')
```
