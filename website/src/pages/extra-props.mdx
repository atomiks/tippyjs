...
// required.
import tippy, { enhance, followCursor } from 'tippy.js'
...
In the browser, the `tippy` module gets automatically configured with all extra
props. Note that treeshaking of separate functions is not possible, so instead
all of the extra props are downloaded. It's recommended to use a bundler instead
of a CDN.
